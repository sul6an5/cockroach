...
func (ds *DistSender) Send(
	...
	for len(parts) > 0 {
		...
		if withParallelCommit {
			log.Infof(ctx, "send withParallelCommit: %v", withParallelCommit)
			rpl, pErr = ds.divideAndSendParallelCommit(ctx, ba, rs, isReverse, 0 /* batchIdx */)
		} else {
			log.Infof(ctx, "send not withParallelCommit: %v", withParallelCommit)
...
func (ds *DistSender) divideAndSendParallelCommit(
	...
	log.Infof(ctx, "divideAndSendParallelCommit len(ba.Requests): %v", len(ba.Requests))
...
func (ds *DistSender) divideAndSendBatchToRanges(
	...
	log.Infof(ctx, "divideAndSendBatchToRanges total: %v", len(ba.Requests))
	...
	if !ri.NeedAnother(rs) {
		...
		if ba.IsSingleRequest() {
			log.Infof(ctx, "single request: %v", len(ba.Requests))
			resp := ds.sendPartialBatch(
				ctx, ba, rs, isReverse, withCommit, batchIdx, ri.Token(), nil, /* positions */
			)
			return resp.reply, resp.pErr

		} else if len(ba.Requests) == 2 && hasET && !args.(*kvpb.EndTxnRequest).Require1PC {
			log.Infof(ctx, "2 request with No Require1PC: %v", len(ba.Requests))

			baSuffix := ba.ShallowCopy()
			baPrefix := ba.ShallowCopy()

			baPrefix.Requests = baPrefix.Requests[:etIdx]
			baSuffix.Requests = baSuffix.Requests[etIdx:]

			resp1 := ds.sendPartialBatch(
				ctx, baPrefix, rs, isReverse, withCommit, batchIdx, ri.Token(), []int{0}, /* positions */
			)
			...
			ba.UpdateTxn(resp1.reply.Txn)
			batchIdx++
			//br := resp1.reply
			if err := br.Combine(ctx, resp1.reply, []int{0}, ba); err != nil {
				return nil, kvpb.NewError(err)
			}
			resp2 := ds.sendPartialBatch(
				ctx, baSuffix, rs, isReverse, withCommit, batchIdx, ri.Token(), []int{etIdx},
			)
			if resp2.pErr != nil {
				pErr = resp2.pErr
				pErr.UpdateTxn(br.Txn)
			...
			if err := br.Combine(ctx, resp2.reply, []int{etIdx}, ba); err != nil {
				return nil, kvpb.NewError(err)
			}
			return br, pErr
			...
			log.Infof(ctx, "args.(*kvpb.EndTxnRequest).Require1PC: %v", args.(*kvpb.EndTxnRequest).Require1PC)
			log.Infof(ctx, "2 Require1PC: %v", len(ba.Requests))
			resp3 := ds.sendPartialBatch(
				ctx, ba, rs, isReverse, withCommit, batchIdx, ri.Token(), nil, /* positions */
			)
			return resp3.reply, resp3.pErr

		} else if len(ba.Requests) > 2 && hasET && args.(*kvpb.EndTxnRequest).Require1PC {
			// if len(ba.Requests) > 1 && hasET && args.(*kvpb.EndTxnRequest).Require1PC
			log.Infof(ctx, "more than 2 Require1PC: %v", len(ba.Requests))
			resp4 := ds.sendPartialBatch(
				ctx, ba, rs, isReverse, withCommit, batchIdx, ri.Token(), nil, /* positions */
			)
			return resp4.reply, resp4.pErr
		} else if len(ba.Requests) > 2 && hasET && !args.(*kvpb.EndTxnRequest).Require1PC {
			// if len(ba.Requests) > 1 && hasET && args.(*kvpb.EndTxnRequest).Require1PC
			log.Infof(ctx, "more key not Require1PC: %v", len(ba.Requests))
			// resp5 := ds.sendPartialBatch(
			// 	ctx, ba, rs, isReverse, withCommit, batchIdx, ri.Token(), nil, /* positions */
			// )
			// return resp5.reply, resp5.pErr

			for i := 0; i < len(ba.Requests)-1; i++ {
				baPrefix := ba.ShallowCopy()
				baPrefix.Requests = baPrefix.Requests[i : i+1]

				resp6 := ds.sendPartialBatch(
					ctx, baPrefix, rs, isReverse, withCommit, batchIdx, ri.Token(), nil, /* positions */
				)
				...
				ba.UpdateTxn(resp6.reply.Txn)
				batchIdx++
				//br := resp1.reply
				if err := br.Combine(ctx, resp6.reply, []int{i}, ba); err != nil {
					return nil, kvpb.NewError(err)
				}
			}
			baPrefix := ba.ShallowCopy()
			z := len(ba.Requests) - 1
			baPrefix.Requests = baPrefix.Requests[z:]
			resp7 := ds.sendPartialBatch(
				ctx, baPrefix, rs, isReverse, withCommit, batchIdx, ri.Token(), nil, /* positions */
			)
			if resp7.pErr != nil {
				pErr = resp7.pErr
			...
			ba.UpdateTxn(resp7.reply.Txn)
			//br := resp1.reply
			if err := br.Combine(ctx, resp7.reply, []int{z}, ba); err != nil {
				return nil, kvpb.NewError(err)
			}
			return br, pErr
		} else if !hasET {
			// if len(ba.Requests) > 1 && hasET && args.(*kvpb.EndTxnRequest).Require1PC
			log.Infof(ctx, "hasET: %v", hasET)
			...
			// if len(ba.Requests) > 1 && hasET && args.(*kvpb.EndTxnRequest).Require1PC
			log.Infof(ctx, "something else: %v", len(ba.Requests))
			resp7 := ds.sendPartialBatch(
				ctx, ba, rs, isReverse, withCommit, batchIdx, ri.Token(), nil, /* positions */
			)
			return resp7.reply, resp7.pErr
		}
	} else {
		log.Infof(ctx, "More than one range %d", len(ba.Requests))
	}
	...
	if ba.Txn == nil {
		log.Infof(ctx, "ba.Txn: %v", len(ba.Requests))
	...
	if withCommit {
		log.Infof(ctx, "With multiple ranges and commit: %v", len(ba.Requests))
		etArg, ok := ba.GetArg(kvpb.EndTxn)
		if ok && !etArg.(*kvpb.EndTxnRequest).IsParallelCommit() {
			return nil, errNo1PCTxn
		}
	...
	for ; ri.Valid(); ri.Seek(ctx, seekKey, scanDir) {
		...
		responseChe := make(chan response, 1)

		// Truncate the request to range descriptor.
		curRangeRS, err := rs.Intersect(ri.Token().Desc().RSpan()) // current range where we at
		if err != nil {
			responseChe <- response{pErr: kvpb.NewError(err)}
			return
		}
		...
		log.Infof(ctx, "len(curRangeBatch.Requests) %v", len(curRangeBatch.Requests))
		for i, req := range curRangeBatch.Requests {
			newBatch := curRangeBatch.ShallowCopy()
			var newpos []int
			newBatch.Requests = make([]kvpb.RequestUnion, 0)
			...
			if canParallelize && !lastRange && !ds.disableParallelBatches &&
				ds.sendPartialBatchAsync(ctx, newBatch, curRangeRS, isReverse, withCommit, batchIdx, ri.Token(), responseCh, newpos) {
				// Sent the batch asynchronously.

				log.Infof(ctx, "Sending partial batch multiple ranges async with: %v", len(newBatch.Requests))
			} else {
				log.Infof(ctx, "Sending partial batch multiple ranges sync with: %v", len(newBatch.Requests))
				resp := ds.sendPartialBatch(
					ctx, newBatch, curRangeRS, isReverse, withCommit, batchIdx, ri.Token(), newpos,
				)
				responseCh <- resp
				if resp.pErr != nil {
					return
